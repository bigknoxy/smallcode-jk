...
export const env = {
  ...
  get watchdogMaxPrompt(): number { return intEnv("SMALLCODE_WATCHDOG_MAX_PROMPT", 8192); },
  get targetLock(): boolean { return boolEnv("SMALLCODE_TARGET_LOCK", true); },
  get targetSet(): boolean { return boolEnv("SMALLCODE_TARGET_SET", false); },
  get setCarousel(): boolean { return boolEnv("SMALLCODE_SET_CAROUSEL", false); },
  ...
  get finalStateGuard(): boolean { return boolEnv("SMALLCODE_FINAL_STATE_GUARD", true); },
  get importGate(): boolean { return boolEnv("SMALLCODE_IMPORT_GATE", true); },
};
...
export const ENV_REGISTRY: EnvVarDoc[] = [
  ...
  { name: "SMALLCODE_WATCHDOG_MAX_PROMPT", parse: "int", default: "8192", description: "Watchdog abstains from its throughput judgement when a request's prompt exceeds this many tokens (above this, wall-clock tok/s is dominated by prefill/prompt-eval and cannot detect decode decay — abstaining prevents false reload-loops at large num_ctx)." },
  ...
  { name: "SMALLCODE_TARGET_SET", parse: "bool", default: "off", description: "Multi-file target set (opt-in). Generalizes the single-file target lock into a BOUNDED editable set: the pinned primary target PLUS the source files it directly imports (one hop, resolvable, repo-local, non-test; capped for wander-safety — src/agent/target-set.ts). Lets a fix that genuinely spans a function and its helper module edit both files while still hard-rejecting edits OUTSIDE the neighborhood. The prompt also widens 'edit ONLY X' to name the whole set so the model knows the fix may span them. Requires SMALLCODE_TARGET_LOCK on (the set replaces the single lock). Default OFF pending A/B on the multifile suite." },
  { name: "SMALLCODE_SET_CAROUSEL", parse: "bool", default: "off", description: "Set-carousel (opt-in; requires SMALLCODE_TARGET_SET, and thus SMALLCODE_TARGET_LOCK, on). When the model STALLS on the current focus file (stallCount reaches STALL_LIMIT) while the oracle is still failing, advances the model's ATTENTION to the next file in the editable import set with a fresh focus prompt and a fresh stall+redraft budget — decomposing one hard cross-file localization into a sequence of single-file localizations so a small model (7b) can solve a genuine multi-file bug it otherwise fixates on (re-emitting the same primary file every turn instead of ever reaching the coupled bug in a helper module). Fires on stall ALONE, not gated on exhausting the redraft budget first — in set-mode the carousel takes PRIORITY over same-file redraft (redraft yields via a `!carouselActive` guard in loop.ts), since redrafting the same file is useless when the real remaining bug is in a different file, and waiting out a redraft cycle first would burn too many turns of a bounded eval budget. Attention-only: does NOT change enforcement, the oracle, or revert — src/agent/carousel.ts, called from src/agent/loop.ts. Default OFF pending A/B on the multifile suite." },
  ...
  { name: "SMALLCODE_IMPORT_GATE", parse: "bool", default: "on", description: "Static import-resolution gate (Lever 2, opt-in): after a FILE:/PATCH: edit lands, extract the import specifiers the edit INTRODUCED and resolve each against ground truth — relative paths via Bun.resolveSync from the file's dir, bare packages against THIS repo's package.json deps + repo-local node_modules (NOT Bun's global install cache, which would let an undeclared-but-machine-cached package pass) — BEFORE the test oracle runs. Any specifier that does not resolve (a hallucinated module, the dogfood `std/strings` failure) reverts that file to its pre-edit content and feeds the model a targeted `IMPORT ERROR — … does not resolve … this repo's dependencies are: …` message that names the deps that DO exist. Proactive fix for the reactive-only R4 path (which only surfaces `Cannot find module` after a full test run, and only when a test imports the edited file), so the model gets a crisp, earlier, more actionable signal instead of looping on the same invented import. Conservative (a declared dep or a repo-local relative/package resolution passes) to keep false-rejects near zero. Only NEW imports are checked; pre-existing unresolved imports are never held against the edit. Deterministic; module in src/verify/import-check.ts. Default ON since 2026-07-05 (set =0 to disable): validated live — fires + reverts a real 3B hallucinated `import ... from \"slugify\"`, zero false-fires on an import-prone task, with the global-cache resolver fix (bare packages resolve against THIS repo's deps, not Bun's global install cache)." },
  { name: "SMALLCODE_FINAL_STATE_GUARD", parse: "bool", default: "on", description: "Final-state regression guard ('never leave the repo worse than found'): last pass after the model loop AND every repair pass end with the run still UNSOLVED. Recaptures the full test baseline on the FINAL disk state and compares to the run-START baseline; if the repo is STRICTLY WORSE (higher red count OR a test failing now that was green/absent at baseline), restores every file the agent touched to its pristine pre-model content (and deletes any brand-new files it created), then re-verifies the restore reached ≤ baseline. Eval-neutral by construction: only fires on unsolved runs and reverts to the seeded-bug start state, so pass/fail is unchanged — it removes broken residue a partial/wandering run would otherwise leave on disk. Deterministic, model-agnostic. Default ON since 2026-07-05 (set =0 to disable): validated eval-neutral, realrepo subset pooled pass@1 0.90 = baseline (unchanged)." },
];
