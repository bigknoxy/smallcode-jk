...
export const env = {
  ...
  get radHint(): boolean { return boolEnv("SMALLCODE_RAD_HINT", false); },
...
export const ENV_REGISTRY: EnvVarDoc[] = [
  ...
  { name: "SMALLCODE_RAD_HINT", parse: "bool", default: "off", description: "Model-side read-after-delete hint: when a failing turn leaves the `X.delete(K); X.set(K, X.get(K))` ordering bug (reads the just-deleted key → stores undefined) on the locked target, surface a targeted hint in the next prompt so the MODEL reorders the read before the delete. Prompt signal only — passes stay attributed to the model, not the harness. Targets the lru-recency floor. Default OFF (measurement lever)." },
