...
>
> **⚠ MULTIPLE OPEN BRANCHES touch this file (reconcile at merge):** `fix/oracle-truncation-safety` (THIS content — oracle guard bug fix + TARGET_SET real-dogfood, PR pending), `feat/coupled-decls-1d` (1d coupled-site stick + refuted hint, PR #141), `docs/handoff-next-move-1d` (START-HERE pointer, PR #140). Each edits HANDOFF differently — keep ALL sections when merging.
...
1d. **COUPLED TWO-SITE INTRA-FILE edit (the residual ceiling, NEW next axis).** Dogfood poke: even after perfect localization, 7b AND 32b FAIL the muldiv feature-add — NOT capability (32b wrote the `FLIPS` table edit correctly), but both MISSED the coupled 2nd site `OP_RE` (the module-level regex that gates which operators the scanner tokenizes) → new FLIPS keys are DEAD → test red. The 2nd site is fully VISIBLE in context (whole file pinned); the gap is the INFERENCE "add operator ⇒ must also extend the tokenizer regex". This is the intra-file analog of the genuine-multifile coupling problem. Also: `pickTargetFunction` aims the patch at the CONSUMER fn (`enumerateComparisonMutations` L118-155), but the fix lives at MODULE scope (FLIPS L53, OP_RE L94). **Lever (dedicated effort, NOT eval-gameable coaching): surface data-flow-COUPLED module-level declarations a target fn consumes, or richer oracle feedback "assertion expects a mutation that isn't produced".** See [[project_multifile_target_set]].
