...
1b. **PLANNER/RETRIEVAL LOCALIZATION on real repos (the true bottleneck).** Retrieval ranked the I/O/orchestration file (loop.ts) over the mechanism's DEFINITION file (operator-mutation.ts) for "operator-mutation repair pass". The scorer needs to prefer the file that DEFINES a named mechanism over files that merely reference/use it. This is the localization ceiling ([[project_r2_mri_probe]], R2) surfacing in the PLANNER on a real 644-file repo — the highest-leverage real lever now.
1c. **FINAL-STATE GUARD hole on the max_turns path.** A failed `smallcode run` left 4 spurious edits unreverted. Verify `SMALLCODE_FINAL_STATE_GUARD` covers the max_turns exit of `smallcode run` (not just the eval loop); a failed run must never leave the tree worse. ([[project_guard_escalation_hole]], [[project_final_state_guard]]).
