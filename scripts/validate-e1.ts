#!/usr/bin/env bun
/**
 * E1 validation script — verifies that all regression eval tasks have a
 * referenceSolution fixture and that each fixture passes its graders.
 *
 * Run: bun scripts/validate-e1.ts
 *
...
const SUITE_DIR = join(PROJECT_ROOT, "evals", "suites", "regression");
const FIXTURES_DIR = join(PROJECT_ROOT, "evals", "fixtures");
const TMP_BASE = join(PROJECT_ROOT, ".tmp-validate-e1");
...
async function main(): Promise<void> {
  console.log("[validate-e1] Loading regression suite...");

  const suite = await loadSuite(SUITE_DIR);
  console.log(`[validate-e1] Found ${suite.tasks.length} tasks\n`);
  ...
  console.log(`\n[validate-e1] Results: ${passCount} pass, ${failCount} fail`);

  if (failCount > 0) {
    process.exit(1);
  }

  console.log("[validate-e1] All reference solutions pass.");
}

main().catch((err: unknown) => {
  console.error("[validate-e1] ERROR:", err instanceof Error ? err.message : String(err));
  process.exit(1);
});
