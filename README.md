...
- `SMALLCODE_RAD_HINT`: **model-side** read-after-delete hint, **off by default** (set `=1` to enable). When a failing turn leaves the `X.delete(K); X.set(K, X.get(K))` pattern (which re-inserts `undefined`) in the model's attempt, the next prompt gets a `## STATEMENT ORDER BUG` section (via `detectReadAfterDelete` in `src/repair/read-after-delete.ts`) telling the model to read the value into a variable BEFORE the delete — the MODEL then reorders and solves, no harness rescue. A/B (`realrepo-lru-recency_1`, 7b, n=8): pass@1 **0.13 → 1.00**, avg turns 5.0 → 2.3, lucky-pass audit modelSolveRate 1.0 / 0 rescued — the first model-side lever to move a genuine realrepo floor.
