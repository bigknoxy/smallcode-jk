...
- `SMALLCODE_PHASE_GATE`: statewright-style phase-gated tool access, **opt-in, default OFF** (set `=1` to enable). While no target is confidently pinned and no file has been read yet ("explore" phase), it advertises only read/inspect tools and hard-rejects `write_file`/`FILE:`/`PATCH:` edits until the model has read the target; a pinned/locked target is always "edit" phase (unchanged behavior). A/B (edit-reliability, n=5): INERT — retrieval always pinned a target so explore never fired (ON ≡ OFF); kept as a knob for low-confidence-retrieval tasks. `src/agent/phase-gate.ts`.
...
- `SMALLCODE_BOOL_REPAIR`: harness-side boolean-mutation repair, **opt-in, default OFF** — E4-T2, the first new archetype built on the pluggable repair interface (`src/repair/archetype.ts`). A DISJOINT bug shape from operator/literal repair: a wrong boolean DEFAULT (code returns/assigns `true` where it should be `false`, or vice-versa — no operator to flip, no integer to perturb). Same gate + structure as `SMALLCODE_MUTATION_REPAIR` (post-loop, unsolved fix-mode run, locked target, non-load-error baseline): `enumerateBooleanMutations` (`src/repair/boolean-mutation.ts`) flips each standalone `true`↔`false` in the locked function, the shared driver runs the real oracle on each and keeps the first full-green (can't fake-green), reverting non-winners; a solve records a `mutationRepair` turn (attributed as a HARNESS rescue). **Kept default OFF pending A/B evidence**: the current 22-task realrepo suite has no wrong-boolean-default floor for it to move, so it is INERT there (0 candidates on non-boolean code by construction) — it's a ready mechanism, promoted only when a real floor appears. Adding it was a `targets`+`candidatesFor` pair on the E4-T1 interface, demonstrating the interface pays off. Set `=1` to enable.
- `SMALLCODE_BOOL_REPAIR_MAX`: caps the number of boolean-mutation candidates `runBooleanRepair` will try per pass (default `60`).
...
- `SMALLCODE_ORACLE_CACHE`: **harness-side** oracle no-change skip (**default OFF**, opt-in pending promotion). A single run spends ~89% of its oracle wall-clock re-running the *whole* `bun test` suite on repo states it has already run — repair-candidate passes re-verify the same tree after reverting a candidate, and the final-state guard re-runs a suite the last turn just ran. With this ON, every suite spawn is keyed by `repoFingerprint()` (bun version + test command + sanitized env + `bunfig.toml`/lockfile + every tracked file's **mode and bytes**); an exact repeat of a key returns the memoized run instead of spawning, and prints `oracle: cached (unchanged repo state)` — a cache that silently skips test runs is the DX failure mode. What is memoized is the raw full-suite **run**, never a finished verdict: the verdict is re-derived from the cached output against the *current* baseline on every call, so a stale baseline can never shape a cached answer. Fails closed everywhere — a `null` fingerprint (git failure, untracked non-ignored file, unreadable file, tracked symlink) always spawns and never stores, and a run killed by the 120s timeout is never stored because its output is a *prefix* of the truth, not the truth. The memo lives for one process only and is never written to disk. Key = (resolved repo **path**, fingerprint), not the fingerprint alone: the fingerprint hashes content, not location, so two directories holding byte-identical repos hash the same — routine in the eval harness, which runs many trials from one fixture template in one process — and a test reading `process.cwd()` or an absolute path would then get another directory's verdict. The memo is capped at 32 entries with LRU eviction (a hit re-inserts), so a long eval run cannot grow it without bound. `SMALLCODE_ORACLE_CACHE=0` is a hard kill switch honored on every path, including when the cache is already warm. Honest limit: any untracked non-ignored file makes the fingerprint `null`, so the hit rate depends on worktree cleanliness. `src/verify/fingerprint.ts` + `src/verify/oracle.ts`.
